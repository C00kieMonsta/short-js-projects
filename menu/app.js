...
window.addEventListener('DOMContentLoaded', () => {
  displayMenu(menu);
  const categories = menu.reduce((prev, next) => {
...
function displayMenu(items) {
  const displayMenu = items.map(generateMenuItem).join('');
...
function displayCategories(items) {
  const categories = items.map(item => {
    return `
    <button class="filter-btn" type="button" data-category=${item}>${item}</button>
    `;
  }).join('');
